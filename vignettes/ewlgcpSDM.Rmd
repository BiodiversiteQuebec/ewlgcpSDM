...
obs<-rgbif::occ_data(scientificName = "Fagus grandifolia", hasCoordinate = TRUE,limit=1500,geometry=wkt)$data
...
plot(log(aggregate(gbif,10,fun=sum)))
...
To avoid predicting high relative abundances in areas where the species is not found, we create a 500 km buffer around locations outside of which we will add a fake effort suggesting that the species was not found. This buffer is optional and these values are arbitrary and can be adapted to specific situations. 
```{r}
...
```{r}
plot(mask(sdm$mean,vect(region)))
